		...
		for (i in 1:length(mcc_tre$annotations))
			{
				...
				mcc_tab[i,c("endLon","endLat")] = cbind(annotations$coordinates2, annotations$coordinates1) # to be edited
				...
				if (length(index) > 0)
					{
						...
						mcc_tab[i,c("startLon","startLat")] = cbind(annotations$coordinates2, annotations$coordinates1) # to be edited
